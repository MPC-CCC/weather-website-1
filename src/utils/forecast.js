...
const forecast = (lat, long, callback) => {
    ...
    request({ url, json: true }, (error, { body }) => {
        if (error) {
            callback('Unable to connect to forecast services. Please check internet connection.', undefined)
        } else if (body.error) {
            callback('Coordinates are incorrect. Please try again.', undefined)
        } else {
